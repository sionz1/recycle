        ...
        if (labels != null) {
            for (EntityAnnotation label : labels) {
                switch (label.getDescription()){
                    ...
                    case "Glasses":
                        message.append(String.format("깨진 유리는 신문지에 싸서 일반쓰레기에 버려주세요"));
                        break;

                    case "Plastic Bag":
                        message.append(String.format("1회용 비닐봉지 내 이물질 제거한 후 흰색과 유색으로 구별하여 배출해주세요 \n" +
                                "(표면이 페인트등으로 도색된 비닐의 경우는 재활용 불"));
                        break;
